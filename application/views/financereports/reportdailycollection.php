        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box removeboxmius">
                    ...
                    <div class="row">
                        <?php
                        if (isset($fees_data) || isset($other_fees_data)) {
                        ?>
                            <div class="" id="transfee">
                                ...
                                <div class="box-body">
                                    ...
                                    <?php
                                    if (!empty($fees_data) || !empty($other_fees_data)) {
                                    ?>
                                        <div class="table-responsive">
                                            ...
                                            <table class="table table-striped table-bordered table-hover example">
                                                ...
                                                <tbody>
                                                    ...
                                                    ?>
                                                    <?php 
                                                    foreach ($other_fees_data as $fee_key => $fee_value) {
                                                        $total_amount += $fee_value['amt'];
                                                    ?>
                                                        <tr>
                                                            <td><?php echo $this->customlib->dateformat(date('Y-m-d', $fee_key)); ?></td>
                                                            ...
                                                            <td class="text text-right" data-order="<?php echo $fee_value['amt']?>"><?php echo $currency_symbol . amountFormat($fee_value['amt']); ?></td>
                                                            <td class="text text-right">
                                                                <button type="button" class="btn btn-default btn-xs otherfee_collection" id="load" data-toggle="tooltip" data-date="<?php echo $fee_key; ?>" data-deposite-id="<?php echo implode(",", $fee_value['student_fees_deposite_ids']); ?>" title="<?php echo $this->lang->line('view_collection'); ?>" data-loading-text="<i class='fa fa-spinner fa-spin'></i>"><i class="fa fa-eye"></i></button>
                                                            </td>
                                                        </tr>
                                                    <?php }?>
...
<script type="text/javascript">
    ...
    $(document).on('click', '.otherfee_collection', function() {
        var $this = $(this);
        var date = $this.data('date');

        $.ajax({
            type: 'POST',
            url: baseurl + "financereports/otherfeeCollectionStudentDeposit",
            data: {
                'date': date,
                'fees_id': $this.data('depositeId'),
            },
