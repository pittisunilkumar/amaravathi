...
class Student extends Admin_Controller
{
    ...
    //     $student_due_feeadding              = $this->studentfeemasteradding_model->getStudentFees($student['student_session_id']);

    //     $data['student_discount_fee'] = $student_discount_fee;
    //     $data['student_due_fee']      = $student_due_fee;
    //     $data['student_due_feeadding']      = $student_due_feeadding;
    ...
    //     // if($staffid){
    //     //     $data['staff'] = $this->staff_model->getProfile($staffid['staff_id']);
    //     // }
    ...
    public function resultexternalexportformat()
    {
        $this->load->helper('download');
        $filepath = "./backend/import/externalresults.csv";
        $data     = file_get_contents($filepath);
        $name     = 'externalresults.csv';

        force_download($name, $data);
    }

    public function resultinternalexportformat()
    {
        $this->load->helper('download');
        $filepath = "./backend/import/internalresults.csv";
        $data     = file_get_contents($filepath);
        $name     = 'internalresults.csv';
    ...
    public function is_unique_roll_no($roll_no) {
        $class_id = $this->input->post('class_id');
        $section_id = $this->input->post('section_id');

        // Check if the combination of class_id, session_id, and roll_no is unique
        $is_unique = $this->student_model->check_sroll_exists($roll_no,$class_id, $section_id);

        if ($is_unique) {
            $this->form_validation->set_message('is_unique_roll_no', 'The roll number for this class and session already exists.');
            return false;
        } else {
            return true;
        }
    ...
    // public function is_dobless15($dob){
    //     $admission_date_str    = $this->input->post('admission_date');
    //     $admission_date = DateTime::createFromFormat('d/m/Y', $admission_date_str);
    //     $dob_str = DateTime::createFromFormat('d/m/Y', $dob);
    //     $diff = $dob_str->diff($admission_date);
    //     $years_diff = $diff->y;
    //     if($years_diff<15){
    //         $this->form_validation->set_message('is_dobless15', 'Date Of Birth Must be Have 15 Years below of Admission Date.');
    //         return false;
    //     }else{
    //         return true;
    //     }
    ...
    //     // Convert admission_date_str to DateTime
    //     $admission_date = DateTime::createFromFormat('d/m/Y', $admission_date_str);
    
    //     // Check if admission_date conversion was successful
    //     if (!$admission_date) {
    //         $this->form_validation->set_message('is_dobless15', 'Invalid admission date format');
    //         return false;
    //     }
    ...
    //     if (!$dob_str) {
    //         $this->form_validation->set_message('is_dobless15', 'Invalid date of birth format');
    //         return false;
    //     }
    ...
    //     // Check if the difference is less than 15
    //     if ($years_diff < 15) {
    //         $this->form_validation->set_message('is_dobless15', 'Date Of Birth Must be 15 Years or More Before Admission Date.');
    //         return false;
    //     } else {
    //         return true;
    //     }
    ...
    //     // Check if the difference is less than 15 and if the year of birth is before the year of admission
    //     if ($years_diff < 15 || $dob_str->format('Y') >= $admission_date->format('Y')) {
    //         $this->form_validation->set_message('is_dobless15', 'Date Of Birth must be 15 Years or More Before Admission Date and Birth Year should be earlier than Admission Year.');
    //         return false;
    //     } else {
    //         return true;
    //     }
    ...
    public function is_dobless15($dob) {
        ...
        $date_formats = ['d/m/Y', 'd-m-Y', 'd-M-Y', 'd.m.Y', 'm-d-Y', 'm/d/Y', 'm.d.Y', 'Y/m/d'];
    
        $admission_date = null;
        $dob_date = null;
    
        // Try to convert admission_date_str to DateTime using each format
        foreach ($date_formats as $format) {
            $admission_date = DateTime::createFromFormat($format, $admission_date_str);
            if ($admission_date !== false) {
                break;
            }
        }
        ...
        // Try to convert dob to DateTime using each format
        foreach ($date_formats as $format) {
            $dob_date = DateTime::createFromFormat($format, $dob);
            if ($dob_date !== false) {
                break;
            }
        }
        ...
        // Calculate the difference in years
        $diff = $dob_date->diff($admission_date);
        $years_diff = $diff->y;
    
        // Check if the difference is less than 15 and if the year of birth is before the year of admission
        if ($years_diff < 15 || $dob_date->format('Y') >= $admission_date->format('Y')) {
            $this->form_validation->set_message('is_dobless15', 'Date Of Birth must be 15 Years or More Before Admission Date and Birth Year should be earlier than Admission Year.');
            return false;
        } else {
            return true;
    ...
    public function check_pdf($file) {
        if (!empty($_FILES['apply_doc']['name'])) {
            $file_extension = pathinfo($_FILES['apply_doc']['name'], PATHINFO_EXTENSION);
            $allowed_extensions = array('pdf');
    
            if (!in_array(strtolower($file_extension), $allowed_extensions)) {
                $this->form_validation->set_message('check_pdf', 'Only PDF files are allowed.');
                return FALSE;
            }
        } else {
            $this->form_validation->set_message('check_pdf', 'Please select a PDF file.');
            return FALSE;
        }
    ...
    public function create()
    {
        ...
        $data['adm_auto_insert']       = $this->sch_setting_detail->adm_auto_insert;
        $data['sroll_auto_insert']     = $this->sch_setting_detail->sroll_auto_insert;
        ...
        $this->form_validation->set_rules('reference_id', $this->lang->line('reference_staff'), 'trim|required|xss_clean');
        
        $this->form_validation->set_rules('apply_doc', $this->lang->line('application_document'), 'callback_check_pdf');


        $this->form_validation->set_rules('dob', $this->lang->line('date_of_birth'), 'trim|required|xss_clean');

        // $this->form_validation->set_rules('dob', $this->lang->line('date_of_birth'), 'trim|required|xss_clean|callback_is_dobless15');
        ...
        $this->form_validation->set_rules(
            'admi_no', $this->lang->line('admi_no'), array(
                'xss_clean',
                array('check_student_admi_no_exists', array($this->student_model, 'check_student_admi_no_exists')),
            )
        );
        ...
        if($this->sch_setting_detail->roll_no){
            if(!$this->sch_setting_detail->sroll_auto_insert){
                $this->form_validation->set_rules('roll_no', $this->lang->line('roll_number'), 'trim|required|xss_clean|callback_is_unique_roll_no');
            }
        }
        ...
        if ($this->form_validation->run() == false) {
            ...
            $admi_noo = $this->input->post('admi_no');
            ...
            if ($insert) {
                ...
                if (!empty($_FILES['apply_doc']['name'])) {
                    $upload_path = "./uploads/application_files/";
                    $file_name = "user_" . $insert_id . "_application.pdf"; // You can customize the filename format
                    $file_path = $this->media_storage->applicationfileupload("apply_doc", $upload_path . $file_name);
                    ...
                    if ($file_path !== null) {
                        ...
                        $this -> student_model->uploadapplicationfiledata($data_image);
    ...
    public function edit($id)
    {
        ...
        if($staffid){
            $data['stafff'] = $this->staff_model->getProfile($staffid['staff_id']);
        }else{
            $data['stafff'] = array();
        }
        
        $admi = $this->student_model->getAdmissionNumber($id);
        $data['admi_no']=$admi;
        ...
        $admino = $this->input->post('admi_no');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $staff_id_ref         = $this->input->post('reference_id');
            ...
            //     'session_id' => $session,
            //     'student_id' => $id,
            //     'staff_id'   => $staff_id_ref,
            // );
            

            // $this->student_model->updatereft($reference_array);
            
            
            if(!empty($admino)){
                $ch = $this->student_model->check_admino_($id);
                if($ch){
                    ...
                    $this->student_model->admi_no_update($admidata,$id);
                    ...
                    $this->student_model->admi_no_add($admidata);
                }
            }
            ...
            if (!empty($_FILES['apply_doc']['name'])) {
                $upload_path = "./uploads/application_files/";
                $file_name = "user_" . $id . "_application.pdf"; // You can customize the filename format
                $file_path = $this->media_storage->applicationfileupload("apply_doc", $upload_path . $file_name);
                ...
                if ($file_path !== null) {
                    ...
                    $this -> student_model->updateapplicationdata($data_image);
