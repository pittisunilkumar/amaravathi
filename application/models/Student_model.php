...
class Student_model extends MY_Model
{
    ...
    public function bulkdelete($students)
    {
        if (!empty($students)) {
            ...
            $sql_delete_files = "SELECT file_path FROM student_application WHERE student_id IN (" . implode(', ', $students) . ")";
            $query_files = $this->db->query($sql_delete_files);
            $application_files = $query_files->result_array();
            ...
            $sql_reference = "DELETE FROM student_reference WHERE student_id IN (" . implode(', ', $students) . ")";
            $query_reference = $this->db->query($sql_reference);

            $sql_applicationfilestable = "DELETE FROM student_application WHERE student_id IN (" . implode(', ', $students) . ")";
            $query_reference = $this->db->query($sql_applicationfilestable);
    ...
    {   
        ...
        $field_variable = implode(',', $field_var_array);
        $this->db->select('fees_discount_approval.student_session_id AS stid,fees_discount_approval.approval_status,classes.id AS `class_id`,student_session.id as student_session_id,students.id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,students.middlename,students.lastname,students.image,  students.mobileno,students.email,students.state,students.city,students.pincode,students.religion,students.dob ,students.current_address,students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code, students.guardian_name, students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.app_key,students.parent_app_key,students.rte,students.gender,vehicles.vehicle_no,transport_route.route_title,route_pickup_point.id as `route_pickup_point_id`,pickup_point.name as `pickup_point`,' . $field_variable)->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('categories', 'students.category_id = categories.id', 'left');
        ...
        $this->db->join('fees_discount_approval','fees_discount_approval.student_session_id=students.id');
        $this->db->where('fees_discount_approval.fees_discount_id',$certifid);
        //$this->db->join('fees_discount_approval','fees_discount_approval.student_session_id=students.id','left');
    ...
    public function getByStudentSessionn($student_session_id,$discountid)
    {
        $this->db->select('fees_discount_approval.fees_discount_id,pickup_point.name as pickup_point_name,student_session.route_pickup_point_id,student_session.transport_fees,students.app_key,vehicle_routes.route_id,vehicle_routes.vehicle_id,transport_route.route_title,vehicles.vehicle_no,hostel_rooms.room_no,vehicles.driver_name,vehicles.driver_contact,hostel.id as `hostel_id`,hostel.hostel_name,room_types.id as `room_type_id`,room_types.room_type ,students.hostel_room_id,student_session.id as `student_session_id`,student_session.fees_discount,classes.id AS `class_id`,classes.class,sections.id AS `section_id`,sections.section,class_sections.id as `class_section_id`,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,students.middlename,  students.lastname,students.image,    students.mobileno, students.email ,students.state ,   students.city , students.pincode , students.note, students.religion, students.cast, school_houses.house_name,   students.dob ,students.current_address, students.previous_school,students.guardian_is,students.parent_id,          students.permanent_address,students.category_id,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name , students.father_pic ,students.height ,students.weight,students.measurement_date, students.mother_pic , students.guardian_pic , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.father_phone,students.blood_group,students.school_house_id,students.father_occupation,students.mother_name,students.mother_phone,students.mother_occupation,students.guardian_occupation,students.gender,students.guardian_is,students.rte,students.guardian_email, users.username,users.password,students.dis_reason,students.dis_note,students.app_key,students.parent_app_key')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('class_sections', 'class_sections.class_id = classes.id and class_sections.section_id = sections.id');
    ...
    // public function lastrollRecord($class_id,$section_id)
    // {
    ...
    //     $last_row = $this->db->select('students.roll_no')->order_by('roll_no', "desc")->limit(1)->get('students')->row();
    //     return $last_row;
    // }
    ...
    public function add_student_reference($data){
    ...
    public function getreferencedetails($student_id){
        
        $this->db->select('student_reference.student_id,student_reference.staff_id')->from('student_reference');
        $this->db->where('student_reference.session_id', $this->current_session);
        $this->db->where('student_reference.student_id', $student_id);
        $query = $this->db->get();
        return $query->row_array();

    }
    
    public function uploadapplicationfiledata($data){
    ...
    public function updatereft($data){
        $result=$this->db->where('student_id',$data['student_id'])->update('student_reference',$data);
        if (!$result) {
            log_message('error', 'Database update error: ' . $this->db->error()['message']);
        }
    }


    public function updateapplicationdata($data){
    
        $result=$this->db->where('student_id',$data['student_id'])->update('student_application',$data);
        if (!$result) {
            log_message('error', 'Database update error: ' . $this->db->error()['message']);
        }
    ...
    public function admi_no_add($data){
    ...
    public function check_admino_id($id){
    ...
    public function check_admino_($id){
    ...
    public function getadmi_no_id_for_hallticket($admi_no){
        $query = $this->db->select('id')
                          ->from('student_admi')
                          ->where('admi_no', $admi_no)
                          ->where('admi_status',1)
                          ->get();
    ...
    public function searchFullTexthole($searchterm, $carray = null)
    {
        $userdata = $this->customlib->getUserData();
        $staff_id = $userdata['id'];
    ...
    // public function totalsearchByClassSectionWithSession($class_id = null, $section_id = null, $session_id = null)
    // {
    //     $this->db->select('classes.id AS `class_id`,student_session.id as student_session_id,students.id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,students.middlename,  students.lastname,students.image,  students.mobileno, students.email ,students.state,students.city , students.pincode,students.religion,students.dob ,students.current_address,students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name, students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.rte,students.gender')->from('students');
    //     $this->db->join('student_session', 'student_session.student_id = students.id');
    //     $this->db->join('classes', 'student_session.class_id = classes.id');
    ...
    public function update_student_session($data,$session)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
