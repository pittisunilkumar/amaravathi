...
class Studentfeemaster_model extends MY_Model
{
    ...
    // public function getStudentadditionalFees($student_session_id)
    // {
    //     $sql = "SELECT `student_fees_master`.*,fee_groups.name FROM `student_fees_master` INNER JOIN fee_session_groups on student_fees_master.fee_session_group_id=fee_session_groups.id INNER JOIN fee_groups on fee_groups.id=fee_session_groups.fee_groups_id  WHERE `student_session_id` = " . $student_session_id . " ORDER BY `student_fees_master`.`id`";
    //     $query = $this->db->query($sql);
    //     $result = $query->result();
    ...
    // public function getStudentadditionalFees($student_session_id)
    // {
    //     $sql = "SELECT `additionalstudent_fees_master`.*,additionalfee_groups.name FROM `additionalstudent_fees_master` 
    //     INNER JOIN additionalfee_session_groups on additionalstudent_fees_master.additionalfee_session_group_id=additionalfee_session_groups.id 
    //     INNER JOIN additionalfee_groups on additionalfee_groups.id=additionalfee_session_groups.fee_groups_id WHERE `student_session_id` = " . $student_session_id . " ORDER BY `additionalstudent_fees_master`.`id`";
    //     $query = $this->db->query($sql);
    //     $result = $query->result();
    //     if (!empty($result)) {
    //         foreach ($result as $result_key => $result_value) {
    ...
    //             $fee_session_group_id = $result_value->fee_session_group_id;
    //             $student_fees_master_id = $result_value->id;
    //             $result_value->fees = $this->getDueadditionalFeeByFeeSessionGroup($fee_session_group_id, $student_fees_master_id);
    ...
    public function getDueadditionalFeeByFeeSessionGroup($fee_session_groups_id, $student_fees_master_id)
    {
        $sql = "SELECT
                    additionalstudent_fees_master.*, 
                    additionalfee_groups_feetype.id AS fee_groups_feetype_id,
                    additionalfee_groups.name,
                    additionalfee_groups_feetype.due_date,
                    additionfeegroupandtypeamount.amount,
                    additionalfee_groups_feetype.fine_amount,
                    additionalfeetype.code,
                    additionalfeetype.type,
                    IFNULL(additionalstudent_fees_deposite.id, 0) AS student_fees_deposite_id,
                    IFNULL(additionalstudent_fees_deposite.amount_detail, 0) AS amount_detail
                FROM
                    additionalstudent_fees_master
                INNER JOIN
                    additionalfee_session_groups ON additionalfee_session_groups.id = additionalstudent_fees_master.additionalfee_session_group_id
                INNER JOIN
                    additionalfee_groups_feetype ON additionalfee_groups_feetype.fee_session_group_id = additionalfee_session_groups.id
                INNER JOIN
                    additionalfee_groups ON additionalfee_groups.id = additionalfee_groups_feetype.fee_groups_id
                INNER JOIN
                    additionalfeetype ON additionalfeetype.id = additionalfee_groups_feetype.feetype_id
                LEFT JOIN
                    additionalstudent_fees_deposite ON additionalstudent_fees_deposite.additionalstudent_fees_master_id = additionalstudent_fees_master.id 
                    AND additionalstudent_fees_deposite.additionalfee_groups_feetype_id = additionalfee_groups_feetype.id
                LEFT JOIN
                    additionfeegroupandtypeamount ON additionfeegroupandtypeamount.additionalfee_session_group_id = additionalfee_session_groups.id
                    AND additionfeegroupandtypeamount.additionalgroupid = additionalfee_groups.id
                    AND additionfeegroupandtypeamount.additionaltypeid = additionalfeetype.id
                
                WHERE
                    additionalstudent_fees_master.id = $student_fees_master_id
                    AND additionalfee_groups_feetype.fee_session_group_id = $fee_session_groups_id";
    ...
    public function getStudentadditionalFees($student_session_id)
    {
        $sql = "SELECT `additionalstudent_fees_master`.*,additionalfee_groups.name FROM `additionalstudent_fees_master`
        INNER JOIN additionalfee_session_groups on additionalstudent_fees_master.additionalfee_session_group_id=additionalfee_session_groups.id
        INNER JOIN additionalfee_groups on additionalfee_groups.id=additionalfee_session_groups.fee_groups_id WHERE `student_session_id` = " .
            $student_session_id . " ORDER BY `additionalstudent_fees_master`.`id`";
        $query = $this->db->query($sql);
        $result = $query->result();
        if (!empty($result)) {
            foreach ($result as $result_key => $result_value) {

                $fee_session_group_id = $result_value->additionalfee_session_group_id;
                $student_fees_master_id = $result_value->id;
                $result_value->fees = $this->getDueadditionalFeeByFeeSessionGroup($fee_session_group_id, $student_fees_master_id);
