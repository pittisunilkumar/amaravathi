...
class Additionalstudentfeemaster_model extends MY_Model
{
    ...
    {
        $sql = "SELECT IFNULL(`additionalstudent_fees_master`.`id`, '0') as `student_fees_master_id`,`classes`.`id` AS `class_id`,"
        ...
        . "ON `students`.`category_id` = `categories`.`id` LEFT JOIN additionalstudent_fees_master on"
        . " additionalstudent_fees_master.student_session_id=student_session.id"
        . "  AND additionalstudent_fees_master.additionalfee_session_group_id=" . $this->db->escape($fee_session_group_id)
    ...
    {       
        ...
        $this->db->where('additionalfee_session_group_id', $data['additionalfee_session_group_id']);
        $q = $this->db->get('additionalstudent_fees_master');

        if ($q->num_rows() > 0) {
            return $q->row()->id;
        } else {
            $this->db->insert('additionalstudent_fees_master', $data);
    ...
    public function assign_bulk_fees($fee_session_group,$student_session_id,$delete_fee_session_group)
    {
        if(!empty($fee_session_group)){
            ...
            $this->db->insert_batch('additionalstudent_fees_master', $data_insert);
        }

        if(!empty($delete_fee_session_group)){
            $this->db->where('student_session_id', $student_session_id);
            $this->db->where_in('fee_session_group_id', $delete_fee_session_group);
            $this->db->delete('additionalstudent_fees_master');
        }
    }
    ...
    public function delete($fee_session_groups, $array)
    {
        ...
        $this->db->where('additionalfee_session_group_id', $fee_session_groups);
    ...
    {
        $sql    = "SELECT `student_fees_master`.*,fee_groups.name FROM `student_fees_master` INNER JOIN fee_session_groups on student_fees_master.additionalfee_session_group_id=fee_session_groups.id INNER JOIN fee_groups on fee_groups.id=fee_session_groups.fee_groups_id  WHERE `student_session_id` = " . $student_session_id . " ORDER BY `student_fees_master`.`id`";
        $query  = $this->db->query($sql);
        $result = $query->result();
        if (!empty($result)) {
            foreach ($result as $result_key => $result_value) {
    ...
    public function studentDeposit($data)
    {
        $sql = "SELECT fee_groups.is_system,student_fees_master.amount as `student_fees_master_amount`, fee_groups.name as `fee_group_name`,feetype.code as `fee_type_code`,fee_groups_feetype.amount,fee_groups_feetype.fine_percentage,fee_groups_feetype.fine_amount,fee_groups_feetype.due_date,IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail` from student_fees_master
               INNER JOIN fee_session_groups on fee_session_groups.id=student_fees_master.additionalfee_session_group_id
    ...
    public function additonalfeeasign($data)
    {
        $this->db->trans_start(); // Starting Transaction

        $this->db->insert('additionfeegroupandtypeamount', $data);
        $id = $this->db->insert_id();
        ...
        if ($this->db->trans_status() === false) {
            ...
            // Commit transaction
            $this->db->trans_complete();
            // Log success message or return inserted ID
            return $id;
        }
    }
