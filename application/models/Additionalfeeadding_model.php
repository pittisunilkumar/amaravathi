...
class Additionalfeeadding_model extends CI_Model
{
    ...
    {
        $this->db->select()->from('additionalfeetypeadding');
    ...
    public function remove($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->where('is_system', 0);
        $this->db->delete('additionalfeetypeadding');
    ...
    public function add($data)
    {
        ...
        $existingRecord = $this->db->get_where('additionalfeetypeadding', $dataa)->row();
        if ($existingRecord) {
        ...
        $existingRecord = $this->db->get_where('additionalfeetypeadding', $dataaa)->row();
        if ($existingRecord) {
            // A record with the same values already exists, handle this situation as needed
            $this->db->trans_rollback();
            return false; // You can return an error message or handle it differently
        ...
        if (isset($data['id'])) {
            ...
            $this->db->update('additionalfeetypeadding', $data);
            ...
            $this->db->insert('additionalfeetypeadding', $data);
    ...
    public function check_data_exists($name, $id)
    {
        ...
        $this->db->where('id !=', $id);
        $query = $this->db->get('additionalfeetypeadding');
        if ($query->num_rows() > 0) {
            return true;
        } else {
            return false;
